                ...
                {participants?.map((p: any) => (
                    <div key={p.bidder._id} className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                        <div className="flex items-start justify-between mb-6">
                            <div className="w-14 h-14 bg-[#1b4332]/5 rounded-2xl flex items-center justify-center text-[#1b4332] font-black text-xl">
                                {p.bidder.first_name?.charAt(0) || p.bidder.username?.charAt(0)}
                            </div>
                            <span className="bg-[#1b4332]/10 text-[#1b4332] text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider">Active</span>
                        </div>
                        ...
                        <button className="w-full mt-8 py-4 bg-gray-50 hover:bg-gray-100 rounded-[20px] text-sm font-black text-gray-900 transition-all flex items-center justify-center gap-2">
                            View Bid History
                            <div className="bg-[#1b4332] text-white w-6 h-6 rounded-lg flex items-center justify-center text-[10px]">
                                {p.totalBids}
                            </div>
                        </button>
                    </div>
